...
class AgentMemory:
    ...
    def __init__(self, collection: chromadb.Collection, ollama_chat_model: str, ollama_base_url: str): # <<<--- ADD OLLAMA PARAMS
    ...
    def retrieve_raw_candidates(self, query: str, n_results: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if actual_n_results == 0: log.debug("Memory collection is empty, cannot retrieve candidates."); return [] # Added log
    ...
    def retrieve_and_rerank_memories(
        ...
        if len(filtered_candidates) <= n_final:
            log.info(f"Fewer candidates ({len(filtered_candidates)}) than requested ({n_final}) after filtering. Returning all (sorted by distance).")
...
**Output Format:** Provide *only* a comma-separated list of the numerical indices (starting from 0) of the {n_final} most relevant memories, ordered from most relevant to least relevant. Example: 3, 0, 7, 5"""
        ...
        fallback_memories = sorted(filtered_candidates, key=lambda m: m.get('distance', float('inf')))[:n_final]
        ...
        try:
            ...
            if not matches:
                log.warning(f"LLM re-ranking response ('{rerank_response}') had no numbers. Falling back to top N.")
                return fallback_memories, user_suggestion_content
            selected_indices = [int(idx_str) for idx_str in matches]
            valid_indices = [idx for idx in selected_indices if 0 <= idx < len(filtered_candidates)]
            valid_indices = list(dict.fromkeys(valid_indices)) # Remove duplicates

            if not valid_indices:
                log.warning(f"LLM re-ranking response ('{rerank_response}') had invalid indices. Falling back to top N.")
                return fallback_memories, user_suggestion_content
            ...
            log.error(f"Error parsing LLM re-ranking indices ('{rerank_response}'): {e}. Falling back to top N.")
            return fallback_memories, user_suggestion_content
        except Exception as e:
            log.exception(f"Unexpected error parsing LLM re-ranking response ('{rerank_response}'): {e}. Falling back to top N.")
            return fallback_memories, user_suggestion_content
