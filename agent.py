...
class AutonomousAgent:
    ...
    def handle_user_suggestion_move_on(self) -> str:
        ...
        current_task_id = self.session_state.get(
            "current_task_id"
        )  # Read outside lock is fine for initial check
        agent_running = (
                ...
                if task and task.status in ["planning", "in_progress"]:
                    ...
                    if flag_set_successfully:
                        ...
                        try:
                            ...
                            log.error(
                                f"Error saving session state after setting move_on flag: {e}"
                            )
                        ...
                        if self._is_running.is_set():
                            try:
                                ...
                                log.error(
                                    f"Error adding move_on suggestion memory after setting flag: {e}"
                                )
                        else:
                            log.info(
                                "Agent paused after setting move_on flag, skipping memory add."
                            )
            ...
            if not raw_steps:
                ...
                raw_steps = [
                    ...
                    and not line.strip().startswith(
                        ("```", "**", "Example", "Note:", "Plan:", "Here is")
                    )
                ]
                if not raw_steps:
                    raise ValueError(
                        "No numbered or simple list found in LLM response."
                    )
    ...
    def generate_thinking(
        ...
        if tool_results:
            ...
            archive_info = (
                f" (Archived previous to: {archived_path})" if archived_path else ""
            )
            # Ensure tool_results is handled gracefully if not a dict
            tool_result_summary = (
                (str(tool_results)[:200] + archive_info)
                if isinstance(tool_results, dict)
                else str(tool_results)[:200]
            )
        ...
        if tool_results:
            ...
            try:
                result_context = json.dumps(
                    tool_results.get("result", tool_results),
                    indent=2,
                    ensure_ascii=False,
                )
            except TypeError:
                log.warning(
                    "Tool result for prompt context was not JSON serializable, using string representation."
                )
        ...
        try:
            ...
            if "use_tool" in action_type_str:
                ...
                action_type_str
            ):  # Handles cases where NEXT_ACTION is present but invalid value
                action = {
                    "type": "error",
    ...
    def execute_tool(
        ...
        if tool_name == "status":
            tool_action = (
                "status_report"  # Status tool doesn't need explicit action param
            )
    ...
    def _summarize_and_prune_task_memories(self, task: Task):
        ...
        if task.cumulative_findings and task.cumulative_findings.strip():
            ...
            if summary_text and summary_text.strip():
                ...
                if task.status == "completed" and not task.result:
                    ...
                    task.result = {
                        "answer": summary_text,
                        "steps_taken": task.current_step_index,
                    }
        ...
        if not task_memories:
            log.info(
                f"No specific step memories found for task {task.id} to potentially prune."
            )
            return  # Already added summary, nothing else to do

        log.debug(
            f"Found {len(task_memories)} step-related memories for task {task.id}."
        )
        ...
        if summary_id and config.DELETE_MEMORIES_AFTER_SUMMARY:
            ...
            if memory_ids_to_prune:
                ...
                if ids_to_delete_final:
                    ...
                    log.info(
                        f"No memories eligible for pruning found for task {task.id} after filtering."
                    )
            else:
                log.info(
                    f"No original step memories found eligible for pruning for task {task.id}."
                )
        elif summary_id:
            log.info(
                f"Summary added ({summary_id}). Deletion of step memories disabled or no memories to prune."
            )
        else:
            log.error(
                f"Failed to add final summary memory for task {task.id}. Pruning skipped."
            )
    ...
    def _execute_step(self, task: Task) -> Dict[str, Any]:
        ...
        if not task.plan or task.current_step_index >= len(task.plan):
            ...
            step_details_for_history.update(
                {
                    ...
                    + 1,  # Indicate step *after* last plan step
            ...
            step_log.append(
                f"--- Task '{task.id}' | Step {step_details_for_history['step']}/{total_steps} (Plan Completed) ---"
            )
            ...
            step_details_for_history["result_summary"] = (
                f"Task completed. Final answer/summary generated: {final_answer_text[:200]}..."
            )
            ...
            if action_type == "error":
                ...
                result_content = tool_output.get(
                    "result", {}
                )  # Result payload is nested
                    ...
                    if isinstance(result_content, dict):
                        ...
                        if "message" in result_content:
                            ...
                            result_display_str = (
                                ...
                                + (
                                    f" (Archived previous: {archived_path})"
                                    if archived_path
                                    else ""
                                )
                            )
                            ...
                            and "query_mode" in result_content
                            and result_content["query_mode"] is True
                        ):
                            ...
                            tool_name == "status" and "report_content" in result_content
                        ):
                ...
                if tool_error:  # Prepend error to summary
                    result_summary_for_history = (
                        ...
                        + (
                            "..."
                            if len(result_summary_for_history) > summary_limit
                            else ""
                        )
                    )
                ...
                mem_params = json.dumps(params)[:500] + (
                    "..." if len(json.dumps(params)) > 500 else ""
                )
                mem_summary = (
                    result_summary_for_history  # Use the already generated summary
                )
                self.memory.add_memory(
                ...
                if tool_status in [
                    ...
                    self.session_state["current_task_retries"] = (
                        0  # Reset step retries on success
                    )
                ...
                task.current_step_index += (
                    1  # Mark step conceptually done before setting status
                )
                self.task_queue.update_task(
        ...
        current_step_final_idx = (
            task.current_step_index if step_status == "processing" else step_num_display
        )
        step_log.append(
        ...
        with self._state_lock:
            ...
            if step_log:
                ...
                log.warning(
                    "Step log was empty, skipping adding step details to history."
                )
        ...
        self._update_ui_state(
            ...
            log="\n".join(
                step_log
            ),  # Keep full step log here for potential future use? Or just rely on console?
    ...
    def process_one_step(self) -> Dict[str, Any]:
        ...
        default_state.update(
            {"status": "idle", "log": "Idle.", "current_task_id": None}
        )
        ...
        try:
            ...
            if not task:
                ...
                log.info(
                    f"Starting task: {task.id} - '{task.description[:60]}...' (Attempt {task.reattempt_count + 1})"
                )
                step_result_log.append(
                    f"Starting task {task.id}: {task.description[:60]}... (Attempt {task.reattempt_count + 1})"
                )
                ...
                self.session_state["current_task_retries"] = (
                    0  # Reset STEP retries for new task/attempt
                )
            ...
            if task and task.status == "planning":
                log.info(
                    f"Task {task.id} is in planning state (Attempt {task.reattempt_count + 1}). Generating plan..."
                )
                step_result_log.append(
                    f"Generating plan for task {task.id} (Attempt {task.reattempt_count + 1})..."
                )
                plan_success = self._generate_task_plan(task)

                if plan_success:
                    log.info(
                        f"Plan generated successfully for task {task.id}. Moving to in_progress."
                    )
                    ...
                    self.session_state["current_task_retries"] = (
                        0  # Reset step retries for execution phase
                    )
                    self.save_session_state()
                    ...
                    log.error(
                        f"Failed to generate plan for task {task.id} (Attempt {task.reattempt_count + 1})."
                    )
                    ...
                    if task.reattempt_count < config.TASK_MAX_REATTEMPT:
                        log.warning(
                            f"Planning failed, but task reattempts remain ({task.reattempt_count + 1}/{config.TASK_MAX_REATTEMPT}). Reflecting and retrying planning."
                        )
                        step_result_log.append(
                        ...
                        if self._reflect_on_error_and_prepare_reattempt(
                            ...
                            log.error(
                                f"Also failed to reset task {task.id} state. Failing permanently."
                            )
                    ...
                    if failed_task_obj:
                        ...
                        log.error(
                            f"Could not retrieve final task object for {task.id} after planning failure."
                        )
                ...
                step_result = self._execute_step(
                    task
                )  # This handles state updates internally now
                # _execute_step returns the complete UI state dictionary
                return step_result  # No need to call _update_ui_state again here

            elif (
                task
            ):  # Task exists but status is unexpected (e.g., completed/failed but still in session state)
                log.warning(
                    f"Task {task.id} found but not in expected state (Status: {task.status}). Resetting session."
                )
                step_result_log.append(
                    f"Task {task.id} state unexpected: {task.status}. Resetting session."
                )
                self.session_state["current_task_id"] = None
                self.session_state["current_task_retries"] = 0
            ...
            if current_task_id:
                ...
                if task_to_fail:
                    ...
                    task_to_fail.reflections = (
                        task_to_fail.reflections or ""
                    ) + f"\nLoop Error: {fail_reason}"
                    self.task_queue.update_task(
                        current_task_id, "failed", reflections=task_to_fail.reflections
                    )
                    self._handle_task_completion_or_failure(
                        task_to_fail
                    )  # Trigger identity revision etc.
                else:
                    log.error(
                        f"Could not find task {current_task_id} to mark as failed after loop error."
                    )
    ...
    def generate_new_tasks(
        ...
        log.info(
            f"Using {'Initial Creative' if use_initial_creative_prompt else 'Standard'} Task Gen Prompt. Max tasks: {max_tasks_to_generate}"
        )
        ...
        try:
            ...
            if list_start == -1 or list_end == -1 or list_end < list_start:
                if (
                    ...
                    json_objects = re.findall(
                        r'\{\s*".*?\}\s*', llm_response, re.DOTALL
                    )
                    if json_objects:
                        log.warning(
                            "LLM response for tasks was not a valid JSON list, but found JSON objects. Attempting to parse."
                        )
                        json_str = f"[{','.join(json_objects)}]"  # Wrap found objects in a list
                    else:
                        raise json.JSONDecodeError(
                            f"JSON list '[]' not found and no JSON objects detected: {llm_response[:300]}...",
                            llm_response,
                            0,
                        )
            else:
            ...
            if not isinstance(suggested_tasks, list):
                log.warning(
                    f"LLM task gen result was not a list: {type(suggested_tasks)}"
                )
                return None
            ...
            current_task_ids_in_batch = (
                set()
            )  # Track IDs added in *this* generation cycle for dependency checks
            ...
            for task_data in suggested_tasks:
                if not isinstance(task_data, dict):
                    log.warning(
                        f"Skipping non-dict item in suggested tasks: {task_data}"
                    )
                    continue
                ...
                if (
                    ...
                    log.warning(
                        f"Skipping task with invalid/missing description: {task_data}"
                    )
                    continue
                ...
                if description.lower() in active_task_descriptions:
                    log.warning(
                        f"Skipping duplicate active task: '{description[:80]}...'"
                    )
                    continue
                ...
                if isinstance(dependencies_raw, list):
                    for dep_id in dependencies_raw:
                        ...
                        if (
                            ...
                            log.warning(
                                f"Dependency '{dep_id_str}' for new task '{description[:50]}...' not found in existing tasks or this batch. Ignoring dependency."
                            )
                elif dependencies_raw is not None:
                    log.warning(
                        f"Invalid 'depends_on' format for task '{description[:50]}...': {dependencies_raw}. Expected list."
                    )
                ...
                new_task_id = self.task_queue.add_task(
                    new_task
                )  # add_task now checks for duplicates
                ...
                if new_tasks_added >= max_tasks_to_generate:
                    log.info(
                        f"Reached max task generation limit ({max_tasks_to_generate})."
                    )
                    break
            ...
            log.exception(
                f"Unexpected error during task generation parsing/validation: {e}"
            )
            return None
    ...
    def _autonomous_loop(self, initial_delay: float = 2.0, step_delay: float = 5.0):
        ...
        while not self._shutdown_request.is_set():
            ...
            if (
                self._is_running.is_set()
            ):  # Check again before sleeping if still running
    ...
    def start_autonomous_loop(self):
        if self._agent_thread and self._agent_thread.is_alive():
            ...
            self._agent_thread = threading.Thread(
                ...
                args=(
                    2.0,
                    config.UI_UPDATE_INTERVAL * 2.0,
                ),  # Make step delay slightly longer than UI refresh
                daemon=True,
            )
    ...
    def get_agent_dashboard_state(self) -> Dict[str, Any]:
        ...
        try:
            tasks_structured = (
                self.task_queue.get_all_tasks_structured()
            )  # Already structured by task manager
            memory_summary = self.memory.get_memory_summary()
            ...
            completed_failed_tasks.sort(
                key=lambda t: t.get("Completed At") or t.get("Failed At", "0"),
                reverse=True,
            )
            ...
            return {
                "identity_statement": self.identity_statement,
                "pending_tasks": tasks_structured.get(
                    "pending", []
                ),  # Already sorted by prio/time in task_manager
                "in_progress_tasks": in_progress_planning_tasks_sorted,  # Combined planning/in_progress, sorted by time
                "completed_tasks": tasks_structured.get(
                    "completed", []
                ),  # Already sorted by time in task_manager
                "failed_tasks": tasks_structured.get(
                    "failed", []
                ),  # Already sorted by time in task_manager
