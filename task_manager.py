...
class TaskQueue:
    ...
    def add_task(self, task: Task) -> Optional[str]:
        ...
        for existing_task in self.tasks.values():
            if (
                ...
                log.warning(
                    f"Skipping task add - potential duplicate description: '{task.description[:60]}...'"
                )
    ...
    def update_task(
        ...
        if task_id in self.tasks:
            ...
            if result is not None:
                task.result = result  # Allow overwriting result
            if reflections is not None:
                task.reflections = reflections  # Allow overwriting reflections
            self.save_queue()
        else:
    ...
    def set_priority(self, task_id: str, priority: int) -> bool:
        if task_id in self.tasks:
            ...
            if task.priority == new_priority:
                log.info(f"Task {task_id} priority is already {new_priority}.")
                return True
            log.info(
    ...
    def get_tasks_by_status(self, statuses: List[str]) -> List[Task]:
        ...
        if not statuses:
            return []
        matched_tasks = [
            task for task in self.tasks.values() if task.status in statuses
        ]
